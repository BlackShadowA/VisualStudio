...
def plotBinsSummary(monoOptBinTable, var_name, bar_fill = 'skyblue', bar_alpha = 0.5, bar_width = 0.5, bar_text_color = 'darkblue', 
    ...
    binSummaryTable = monoOptBinTable.loc[monoOptBinTable['varname'] == var_name].sort_values('end')
    ...
    ax2.plot(np.arange(len(binSummaryTable['end'])), binSummaryTable['event_rate'], color=line_color, label='Event Rate', linewidth = line_width)
    ax2.scatter(np.arange(len(binSummaryTable['end'])), binSummaryTable['event_rate'], color=dot_color, s = dot_size)
    ax2.set_xticks(ticks = np.arange(len(binSummaryTable['end'])), labels = binSummaryTable['end'])
    ax2.set_ylabel('Event Rate', color=dot_color)
    
    # Add text
    med = binSummaryTable['event_rate'].median()
    if binSummaryTable.iloc[-1, 4] - binSummaryTable.iloc[0, 4] > 0 :
        for i, val in enumerate(binSummaryTable['event_rate']):
            if val <= med :
    ...
    # Set Legend        
    plt.legend(labels = ['Bar Text : obs_dist', 'Dot Text : event_rate'], loc = 'lower center')
    # Set title
